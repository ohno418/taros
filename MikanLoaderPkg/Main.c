...
EFI_STATUS EFIAPI UefiMain(
   ...
   *   1. Load kernel.elf into temporary memory area.
   *   2. Read its program headers.
   *   3. Load the LOAD segments of kernel.elf into memory.
   */
  ...
  // Load kernel file into temporary memory area.
  VOID* kernel_buffer;
  ...
  // Detect load address range.
  Elf64_Ehdr* kernel_ehdr = (Elf64_Ehdr*)kernel_buffer;
  UINT64 kernel_first_addr, kernel_last_addr;
  CalcLoadAddressRange(kernel_ehdr, &kernel_first_addr, &kernel_last_addr);

  // Allocate kernel memory size.
  ...
  // Load the LOAD segments into memory.
  CopyLoadSegments(kernel_ehdr);
  Print(L"Kernel: 0x%0lx - 0x%0lx\n", kernel_first_addr, kernel_last_addr);

  // Free the temporary memory area.
  ...
  /* Boot kernel */
  // Compute the entrypoint address ("KernelMain()").
  UINT64 entry_addr = *(UINT64*)(kernel_first_addr + 24);

  // Build frame buffer config.
